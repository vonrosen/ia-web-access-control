...
public class AdminController extends AbstractController {
    protected HibernateRuleDao ruleDao;
    private static final DateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    protected static final long NEW_RULE = -1L;
    static {
    ...
    protected ModelAndView ruleList(String surt, Long editingRuleId, HttpServletRequest request,
        ...
        for (Rule rule: rules) {
            ...
            if (comparison <= 0) {
                DisplayRule displayRule = getNewDisplayRule(rule, comparison != 0);
        ...
        if (editingRuleId != null && editingRuleId == NEW_RULE) {
            Rule rule = getNewRule();
            rule.setId(NEW_RULE);
            rule.setSurt(surt);
            
            DisplayRule newRule = getNewDisplayRule(rule, false);// new DisplayRule(rule, false);
